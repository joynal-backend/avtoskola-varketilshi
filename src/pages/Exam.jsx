  ...
  {Array.from({ length: examQuestions.length }).map((_, index) => {
    ...
    return (
      <div
        ...
        onClick={() => isAnswered && setHoveredQuestionIndex(index === hoveredQuestionIndex ? null : index)}
      >
        {/* Question preview popup - only shown for answered questions */}
        {hoveredQuestionIndex === index && isAnswered && (
          <div className="fixed md:absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 z-50 w-[90vw] md:w-80 bg-white shadow-2xl overflow-hidden border-8 border-white"
            style={{
